...
export default function AppPage() {
  ...
  const askMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setResult(data);
      // Refresh profile to update free questions count
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-border/50">
        <button onClick={() => navigate("/")} className="font-serif text-xl font-semibold text-foreground hover:opacity-70 transition-opacity" data-testid="nav-home">
          Case Map
        </button>
        <div className="flex items-center gap-4">
          {profile && profile.subscriptionStatus !== "active" && (
            <span className="text-xs text-muted-foreground" data-testid="text-free-remaining">
              {profile.freeQuestionsRemaining} free question{profile.freeQuestionsRemaining !== 1 ? "s" : ""} remaining
            </span>
          )}
          {profile && profile.subscriptionStatus === "active" && (
            <span className="text-xs text-green-700 dark:text-green-400" data-testid="text-subscribed">Subscribed</span>
          )}
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 flex flex-col lg:flex-row max-w-7xl mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col p-6 lg:p-8 min-w-0">
          ...
          {result && !askMutation.isPending && (
            <div className="flex-1 overflow-auto" data-testid="panel-answer">
        ...
        {/* Right: Sources panel */}
        <aside className="w-full lg:w-80 xl:w-96 border-t lg:border-t-0 lg:border-l border-border/50 bg-card/30 p-6 lg:p-6 overflow-auto" data-testid="panel-sources">
          ...
          {result && result.sources && result.sources.length > 0 ? (
            <div className="space-y-3">
              {result.sources.map((source, i) => (
                <div key={i} className="p-3 bg-background border border-border/60 rounded-md" data-testid={`source-card-${i}`}>
...
/**
 * Very simple markdown to HTML converter for the answer panel
 */
